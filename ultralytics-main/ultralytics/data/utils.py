...
def check_det_dataset(dataset: str, autodownload: bool = True) -> dict[str, Any]:
    ...
    # Load dynamically generated names from names.json if the download script created one.
    # This ensures names override persists across re-runs even when the download script is skipped
    # (because val path already exists from a previous run).
    names_file = path / "names.json"
    if names_file.exists():
        try:
            with open(names_file, encoding="utf-8") as nf:
                loaded = json.load(nf)
            data["names"] = {int(k): v for k, v in loaded.items()}
            data["nc"] = len(data["names"])
            LOGGER.info(f"Loaded {data['nc']} class names from {names_file}")
        except Exception as e:
            LOGGER.warning(f"Failed to load names from {names_file}: {e}")
